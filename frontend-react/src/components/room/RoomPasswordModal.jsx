  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl w-full max-w-xs p-6 shadow-2xl animate-in zoom-in-95 duration-200">
        <h2 className="text-lg font-bold text-slate-800 mb-4 text-center">私密小组</h2>
        <p className="text-sm text-slate-500 text-center mb-6">请输入密码加入该小组。</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-2 pt-2">
             ...
             >
               {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : '加入'}
             </Button>
          </div>
        </form>
      </div>
